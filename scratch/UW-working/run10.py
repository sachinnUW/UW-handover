        ...
        if runMode == "DQN":
            ...
            if data.env.packetRxFlag == 1:
                ...
                if data.env.hoInProgress == 1:
                    ...
                    for i in range(len(recievedPacketRecordFull[2*(data.env.packetReceiverId-1)]) - 1, -1, -1):
                    #print(i)
                        if recievedPacketRecordFull[2*(data.env.packetReceiverId-1)][i] >= round(data.env.time,3) - binWidth2:
                            dataReceived = dataReceived + recievedPacketRecordFull[2*(data.env.packetReceiverId-1)+1][i]
                        else:#if this happens then we are outside of the averaging window, stop calculation
                            break
                    throughputRecord[2*(data.env.packetReceiverId-1)+1].append(dataReceived/binWidth2)
                    #cur_index = data.env.packetReceiverId-1
                    #prev_index = cur_index
                    #data_Sum = throughputRecord[2*cur_index+1]
                    #while ((throughputRecord[2*cur_index] - throughputRecord[2*prev_index] <= G) & (prev_index > 0)):
                    #    prev_index = prev_index - 1
                    #    data_Sum = data_Sum + throughputRecord[2*prev_index+1]
                    #dqn.reward = dataSum/(binWidth*(cur_index-prev_index + 1))
                    dqn.reward = dataReceived/binWidth2
