...
class mlOutput(Structure):
    _pack_ = 1
    _fields_ = [("tttAdjustment", c_double), ("hystAdjustment", c_double)]
...
if runMode == "DQN":
    ...
    action_space1 = [0, 40, 64, 80, 100, 128, 160, 256, 320, 480]
    action_space2 = [0, 1, 3, 5, 7, 9]    
...
exp.reset()
r1 = Ns3AIRL(1358, mlInput, mlOutput)
    ...
    with r1 as data:
        ...
        if runMode == "DQN":
            ...
            if imsi not in all_dqn:
                dqn = DQN(state_size=4, n_actions = len(action_space1) * len(action_space2),loss_val=loss_val, batch_size=1)
                all_dqn[imsi] = dqn
            else:
                dqn = all_dqn[imsi]
            ...
            if data.env.hoInProgress == 1:
                ...
                data.act.tttAdjustment = action_space1[dqn.action_index%len(action_space1)]
                data.act.hystAdjustment = action_space2[int(dqn.action_index/len(action_space1))]
            ...
            if data.env.packetRxFlag == 1:
                ...
                if data.env.hoInProgress == 1:
                    ...
                    each_act[2*(data.env.packetReceiverId-1)+1].append(dqn.action_index)
