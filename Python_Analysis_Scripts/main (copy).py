...
binWidth = 20*.001;#number of points to average over
	...
	for j in range(numUE):
		if len(packetData[i][j].keys()) > 0:
			...
			for k in range(len(dataRateTime[i][j])):
				if dataRateTime[i][j][k] == packetTime[a]:
					recievedBytes2.append(recievedBytes[a])
					a = a + 1
			for k in range(len(dataRateTime[i][j])):
				print("          " + str(dataRateTime[i][j][k]))
				if dataRateTime[i][j][k] < binWidth:
					temp = list(np.where((np.array(packetTime) <= dataRateTime[i][j][k]))[0])
					dataRate[i][j].append(sum([recievedBytes[temp[x]]/(binWidth) for x in range(len(temp))]))
				else:
					temp = list(np.where((np.array(packetTime) <= dataRateTime[i][j][k]) & (np.array(packetTime) >= dataRateTime[i][j][k]-binWidth))[0])
					dataRate[i][j].append(sum([recievedBytes[temp[x]]/(binWidth) for x in range(len(temp))]))
